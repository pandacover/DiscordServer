...
class mainCog(commands.Cog):
        ...
        async with ctx.channel.typing():
            ...
            if member is not None and not member.bot:
                ...
                embed.add_field(
                    ...
                    value=
                    f"{monthList[member.joined_at.month-1]} {member.joined_at.day}, {member.joined_at.year}",
                    inline=False)
                embed.add_field(
                    name="Joined Discord At",
                    value=
                    f"{monthList[member.created_at.month-1]} {member.created_at.day}, {member.created_at.year}",
                    inline=False)
                ...
                embed.add_field(
                    name="Joined Server At",
                    value=
                    f"{monthList[ctx.author.joined_at.month-1]} {ctx.author.joined_at.day}, {ctx.author.joined_at.year}",
                    inline=False)
                embed.add_field(
                    name="Joined Discord At",
                    value=
                    f"{monthList[ctx.author.created_at.month-1]} {ctx.author.created_at.day}, {ctx.author.created_at.year}",
                    inline=False)

            if ctx.author.bot:
                embed.add_field(name="Stupid!",
                                value="Bot don't need profile!")
    ...
    async def serverinfo(self, ctx):
        ...
        async with ctx.channel.typing():
            ...
            embed.set_author(name=f"{guild.name}'s information.",
                             icon_url=ctx.author.avatar_url)
            ...
            embed.add_field(
                name="Created at:",
                value=
                f"{monthList[guild.created_at.month-1]} {guild.created_at.day}, {guild.created_at.year}"
            )
            embed.set_footer(text='Created by OR Dev Team.')
            await asyncio.sleep(0.5)
            await ctx.send(embed=embed)
    ...
    async def stat(self, ctx):
        async with ctx.channel.typing():
            ...
            user = ctx.guild.get_member(745258527031033937)
            current_time = time.time()
            difference = int(round(current_time - start_time))
            text = str(datetime.timedelta(seconds=difference))
            embed = discord.Embed(color=discord.Color.green())
            embed.add_field(
                name="Otaku Bot Status:",
                value=
                f'```java\n   [name] : "{user.name}"\n [uptime] : "{text}"\n [status] : "{user.status}"\n  [cache] : "{len(self.bot.cached_messages)}"  \n[latency] : "{round(self.bot.latency * 1000)}ms"\n  [guild] : "{ctx.author.guild}  |  {guilds}"```'
            )
